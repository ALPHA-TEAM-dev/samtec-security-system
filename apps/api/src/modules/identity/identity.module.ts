 ...
 *
 * The two `APP_GUARD` lines protect the whole API, in order: first the
 * sign-in wall (`AccessTokenGuard`), then the role check (`RolesGuard`).
 * Because they are global, every new endpoint anywhere in the API requires
 * sign-in unless it is explicitly marked `@Public()`.
 */
@Module({
